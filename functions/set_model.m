...
%   random seed. fix it!
% Modified by Long Li 2018-11-08
%   - added the parameter 'ens_forecast'.
% Modified by Long Li 2020-12-02
%   - added 'SVDdata' noise model. 
...
% - 'SVDfull': noise basis on-line learned from pseudo-observations, built by SVD.
% - 'SVDdata': noise basis off-line learned from high-resolution data.
% - 'WavHypervis': noise designed such that its dissipative term -div(a.grad(b))
...
seed = 19920829; % positive integer.
...
folder_output = 'outdata'; % root directory for output data
plot_moments = 0; % plot one-point one-time moments each day (requires plot_results)
image_format = 'png'; % format of output images - see "print formattype".
                      % Use 'epsc' for color EPS files.
image_dpi = 200; % resolution (dot per inch) of output images.
...
switch type_noise
    ...
    case 'SVDfull'
        ...
        divfree_projection = false; %[WIP] not fully testedz
        %-----------------------------------------
        % Gather sigma parameters in one structure
        % NB: do NOT edit values in the remaining of this section
    ...
    case 'WavHypervis'
        ...
        scaling = .5; %scale the energy brought by the noise w.r.t
                      %the energy removed by hyperviscosity
        wavelab = fullfile(matlabroot,'toolbox/Wavelab850');              
        addpath(wavelab); WavePath;
        % Optional: if use parallel computing, then do the following; else
        % comment it
        poolobj = gcp;        
        addAttachedFiles(poolobj,wavelab);
        %-----------------------------------------
        % Gather sigma parameters in one structure
        % NB: do NOT edit values in the remaining of this section
        sigma.k_c = k_c; % [TODO] remove?
    ...
    case 'SVDdata'
        % Model using a noise basis built off-line from HR data by SVD.
        %-----------------------------------------
        k_c = 1./(3e2); % keep it to rescale the noise amptitude 
        file_mod = './POD/data/vort128/EOF_data.mat'; % File of off-lined basis
        ID_mod1 = 1; % Index of 1st mode used for the noise 
        N_mod = 100; % Number of truncated modes for the noise
        divfree_projection = false; 
        %-----------------------------------------
        % Gather sigma parameters in one structure
        sigma.k_c = k_c;
        sigma.file_mod = file_mod;
        sigma.ID_mod1 = ID_mod1;
        sigma.N_mod = N_mod;
        sigma.divfree_projection = divfree_projection;
        sigma.long_name = 'High-resolution observations noise';
    case {'None', ''}
...
switch type_rand
    ...
    case 'SVDnoise'
        ...
        init.N_obs = 20; %number of pseudo-observations to generate
