...
%   - added other colormaps from matplotlib library;
%   - added new noise model - 'SVDdata': noise basis learned from data.
% TODO:
...
model.output.colormap = getPyPlot_cMap('RdBu_r'); 
% see other options in 'functions/output/getPyPlot_cMap.m'
% [TODO] Move colormap choice to set_model
% Version of matlab
vers = version;
...
% [TODO] use this to rescale amptitude of inhomogeneous noise
...
switch model.sigma.type_noise
    ...
    case 'SVDfull'
        ...
        clear D1 D2
    case 'SVDdata'
        % Trucate the spatial modes weighting by the eigenvalues for the 
        % noise and derive the staionary variance tensor.
        model = SVDdata_init(model);
        % [TODO] Keep them as global variables?
        % Periodic gradient operators
        [D1, D2] = gradient_periodic(model.grid.MX(1), model.grid.MX(2), ...
                                     model.grid.dX(1), model.grid.dX(2));
        model.sigma.D1 = D1; clear D1
        model.sigma.D2 = D2; clear D2       
    case 'Hypervis' %[WIP]
...
%output_frequency = ceil(3600/model.advection.dt_adv);
...
if model.verbose
    ...
    %        model.output.frequency*model.advection.dt_adv/3600.);
    fprintf(1, 'Output frequency: %d steps (%.3f days)\n', model.output.frequency, ...
                ...
                switch model.sigma.type_noise
                    ...
                    case 'SVDfull'
                        ...
                        s_sigma_dBt_over_dt = model.sigma.noise_generator.draw_noise(...
                            tmp_U, tmp_S);
                    % High-resolution data noise (anisotropic, inhomogeneous in space but staionary in time)
                    case 'SVDdata'
                        s_A = bsxfun(@times, model.sigma.A_over_dt, model.advection.dt_adv);
                        s_sigma_dBt_over_dt = SVDdata_updt(model);
                        %s_A = bsxfun(@times, model.sigma.a0_on_dt, model.advection.dt_adv);
                        %s_sigma_dBt_over_dt = SVDdata_updt(model, model.sigma.sigma_on_sq_dt);
                        % [TODO] Scaling them by correlation time?
...
function output()
    ...
    %output_file = fullfile(model.output.folder_simu, 'files', file_indexh);
